...
function enc(o, structValue = false, htmlDecodeLayers = 0) {
    if (o == null) {
        return "";
    }
        ...
        return `{${(0, Object_1.keys)(o)
            .map((k) => enc(k, true, htmlDecodeLayers) +
            "=" +
...
class WriteTask extends ExifToolTask_1.ExifToolTask {
    ...
    static for(filename, tags, options) {
        ...
        for (const key of (0, Object_1.keys)(tags)) {
            (0, TagNameValidation_1.validateTagName)(key);
            const val = tags[key];
     ...
     */
    static forTagEdits(filename, edits, options) {
        for (const arg of (0, Array_1.toArray)(options.writeArgs)) {
            if (/^-(?:api|sep)/i.test(arg)) {
                throw new Error(`writeArgs argument is not compatible with exact tag edits: ${arg}`);
            }
        }
        const validatedEdits = (0, TagEdit_1.validateTagEdits)(edits);
        const fieldsToSet = validatedEdits.map((edit) => {
            ...
            const htmlDecodeLayers = (0, Struct_1.isStruct)(value) ? 2 : 1;
            return `-${edit.tag}${operator}=${enc(value, false, htmlDecodeLayers)}`;
        });
        return WriteTask.#forFields(filename, fieldsToSet, options);
    }
    static #forFields(filename, fieldsToSet, options) {
